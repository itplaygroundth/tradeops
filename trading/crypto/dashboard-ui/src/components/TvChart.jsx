...
import { priceDecimals } from '../lib/crypto.js'
...
export default function TvChart({ symbol, timeframe = '15m', height, showVolume = true, showCrosshair = true }) {
  ...
  useEffect(() => {
    ...
    if (showCrosshair) {
      chart.subscribeCrosshairMove(param => {
        ...
        const d = priceDecimals(c.close ?? c.open ?? 0)
        setTooltip({
          open: c.open?.toFixed(d),
          high: c.high?.toFixed(d),
          low: c.low?.toFixed(d),
          close: c.close?.toFixed(d),
          isUp: c.close >= c.open,
        })
      })
    }
  ...
  useEffect(() => {
    ...
    fetch(`/api/ohlcv/${symbol}?timeframe=${timeframe}&count=200`)
      .then(r => r.json())
      .then(raw => {
        if (cancelled) return
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!candleRef.current) return
      fetch(`/api/ohlcv/${symbol}?timeframe=${timeframe}&count=2`)
        .then(r => r.json())
        .then(raw => {
          if (cancelled) return
          const candles = parseOhlcv(raw)
