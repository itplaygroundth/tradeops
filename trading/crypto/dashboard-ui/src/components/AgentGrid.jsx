import { symLabel, fmtUsdt } from '../lib/crypto.js'
...
function AgentCard({ agent }) {
  ...
  return (
    <div className={`agent-card ${cardClass}`}>
      ...
      <div className="agent-strategy">{symLabel(agent.symbol)} · {strat}</div>
      <div className={`agent-pnl ${pnlClass} pnl-blink`} style={{ animationDelay: `${blinkDelay}s`, animationDuration: `${blinkSpeed}s` }}>
        {pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}%
      </div>
      <div className="agent-equity">{agent.total_pnl >= 0 ? '+' : ''}{fmtUsdt(agent.total_pnl || 0)} USDT</div>
